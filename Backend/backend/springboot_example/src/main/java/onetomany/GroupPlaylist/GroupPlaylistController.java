...
public class GroupPlaylistController {
    ...
    // Create a new playlist and add users to it
    @PostMapping
    public ResponseEntity<String> createPlaylist(@RequestBody CreatePlaylistRequest playlistRequest) {
        String playlistName = playlistRequest.getName();
        List<String> userEmails = playlistRequest.getUsers();

        if (playlistName == null || playlistName.trim().isEmpty()) {
            return ResponseEntity.badRequest().body("{\"error\": \"Playlist name is required\"}");
        }

        // Create the playlist object
        GroupPlaylist playlist = new GroupPlaylist(playlistName);

        // If user emails are provided, fetch actual User entities and add them to the playlist
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body("{\"message\": \"Playlist created with name: " + playlistName + "\"}");
    }
